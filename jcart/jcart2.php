...
class Jcart {
	...
	* @return mixed
	*/
	private function add_item($id, $name, $price, $qty = 1, $url, $type) {
	...
	*/
	public function update_cart() {
		...
		$this->zone = $_POST["jcartZone"];
		$this->reg = $_POST["jcartReg"];
	...
	private function update_subtotal() {
		...
		if(sizeof($this->items > 0)) {
			foreach($this->items as $item) {
				...
				$this->weight += ($this->qtys[$item] * $config['types_weights'][$this->types[$item]]) ;
				if ($this->weight > 2000) {
					$this->shipping = $costs[$this->reg][$this->zone]['2001-'];
				} elseif ($this->weight > 1000) {
					$this->shipping = $costs[$this->reg][$this->zone]['1001-2000'];
				} elseif ($this->weight > 500) {
					$this->shipping = $costs[$this->reg][$this->zone]['501-1000'];
				} elseif ($this->weight > 0) {
					$this->shipping = $costs[$this->reg][$this->zone]['0-500'];
				} else {
					$this->shipping = 0;
				}
	...
	*/
	public function display_cart() {
		...
		// Add an item
		if ($_POST[$add]) {
			$itemAdded = $this->add_item($id, $name, $price, $qty, $url, $type);
		...
		echo tab(7) . "<span id='jcart-subtotal'>{$config['text']['subtotal']}: <strong>" . number_format($this->subtotal, $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "$currencySymbol</strong></span>\n";
		echo tab(6) . "</th>\n";
		echo tab(5) . "</tr>\n";
		echo tab(4) . "</tfoot>\n";			
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
				...
				echo tab(7) . "<input name='jcartType[]' type='hidden' value='{$item['type']}' />\n";
				...
				echo tab(7) . "<span id='jcart-subtotal'>" . number_format($item['subtotal'], $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "$currencySymbol</span><input name='jcartItemPrice[]' type='hidden' value='{$item['price']}' />\n";
...
if ($isCheckout === true) {
		...
		echo tab(5) . "<tr>\n";
		echo tab(6) . "<th colspan='3'>\n";
		echo tab(7) . "<strong id='jcart-title'>{$config['text']['shipping']}</strong> ($this->weight gr)\n";
		echo tab(6) . "</th>\n";
		echo tab(5) . "</tr>". "\n";
		echo tab(4) . "</thead>\n";
		//tabla shipping body
		...
		echo tab(7) . "	<ul>\n";
		//echo "<-- ".print_r($config)."-->";
		echo tab(8) . "<ui>{$config['text']['typeshipment']} <select name='jcartReg' >";
		foreach ($config['shipping_types'] as $typeid => $typetext ) {
			echo tab(9) . "<option value='$typeid' ";
			if (($this->reg) == $typeid) {
				echo "selected";
			}
			echo ">$typetext</option>\n";
		}
		echo tab(8) . "</select></ui>\n";
		echo tab(8) . "<ui>{$config['text']['zonetitle']} <select name='jcartZone' class='jcart-buttons'>";
		foreach ($config['shipping_zones'] as $zoneid => $zonetext ) {
			echo tab(9) . "<option value='$zoneid' ";
			if (($this->zone) == $zoneid) {
				echo "selected";
			}
			echo ">$zonetext</option>\n";
		}
		echo tab(8) . "</select></ui>\n";
		echo tab(7) . "	</ul>\n";		
		...
		echo tab(6) . "</td>\n";
		echo tab(6) . "<td class='jcart-item-price'>\n";
		echo tab(7) . "<span>" . number_format($this->shipping, $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "$currencySymbol</span><input name='jcartShipping' type='hidden' value='{$this->shipping}' />\n";
		//echo tab(7) . "<a class='jcart-remove' href='?jcartRemove={$item['id']}'>{$config['text']['removeLink']}</a>\n";
		echo tab(6) . "</td>\n";
		echo tab(5) . "</tr>\n";
		echo tab(4) . "</tbody>\n";
		...
		echo tab(5) . "<tr>\n";
		echo tab(6) . "<th colspan='3'>\n";
		echo tab(7) . "<span id='jcart-subtotal'>{$config['text']['total']}: <strong>" . number_format($this->subtotal+$this->shipping, $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "$currencySymbol</strong></span>\n";
		echo tab(6) . "</th>\n";
		echo tab(5) . "</tr>\n";
		echo tab(4) . "</tfoot>\n";
		
		echo tab(3) . "</table>\n\n";
} else { // if not in checkout maintain the reg and zone values hidden 
		if ($_POST['jcartZone'] || $_POST['jcartReg']) {
			echo tab(3) . "<input type='hidden' name='jcartZone' value='".$_POST['jcartZone']."'>";
			echo tab(3) . "<input type='hidden' name='jcartReg' value='".$_POST['jcartReg']."'>";
		} else {
			echo tab(3) . "<input type='hidden' name='jcartZone' value='".$this->zone."'>";
			echo tab(3) . "<input type='hidden' name='jcartReg' value='".$this->reg."'>";
		}	
}//if is checkout table shipping		
